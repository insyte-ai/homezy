/**
 * Messages Screen (Pro View)
 * Shows list of conversations with homeowners
 */
...
function ConversationItem({
  ...
  // Get the other participant (homeowner for pro view)
  const otherUser = conversation.participants.homeownerId;
  const displayName = `${otherUser.firstName} ${otherUser.lastName}`;

  // Get unread count for pro
  const unreadCount = conversation.unreadCount.professional;
...
export default function MessagesScreen() {
  ...
  const handleConversationPress = (conversation: Conversation) => {
    router.push(`/(pro)/conversation/${conversation.id}`);
  };
    ...
    <SafeAreaView style={styles.container} edges={['top']}>
      ...
      {conversationsLoading && conversations.length === 0 ? (
        ...
        <EmptyState
          ...
          description="When you claim leads and communicate with homeowners, your conversations will appear here."
        />
      ) : (
        <FlatList
          data={conversations}
